  ...
  return (
    <main className="min-h-screen bg-white flex flex-col">
      ...
      <div className="pt-64 md:pt-72 pb-12 px-6 max-w-7xl mx-auto w-full">
        <button 
          onClick={() => router.back()}
          className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] text-[#C4141A] hover:text-black transition-all mb-12 group"
        >
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
          <div>
            <div className="flex items-center gap-2 mb-6">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.4em] text-[#C4141A]">{car.name}</span>
            </div>
            ...
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2 text-black/40 uppercase tracking-widest text-[10px] font-black">
                <MapPin size={14} className="text-[#C4141A]" /> {car.specs.location || "Visakhapatnam"}
              </div>
          ...
          <div className="flex flex-col md:items-end">
            <p className="text-4xl md:text-6xl font-sans font-bold text-[#C4141A] tracking-tighter mb-2 whitespace-nowrap flex items-center gap-4">
            ...
            <div className="grid grid-cols-5 md:grid-cols-6 gap-4">
              {allMedia.map((media, idx) => (
                <button 
                  ...
                  className={`aspect-square rounded-2xl md:rounded-[2rem] overflow-hidden border-2 transition-all ${activeMedia.url === media.url ? 'border-[#C4141A]' : 'border-transparent opacity-60 hover:opacity-100'}`}
                >
                  <div className="relative w-full h-full">
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="p-10 bg-gray-50 border border-black/5 rounded-[3.5rem] shadow-xl">
              ...
              <button 
                ...
                className="w-full py-6 bg-[#C4141A] text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-black transition-all shadow-xl active:scale-95"
              >
                Inquire for Best Price
              </button>
              <div className="mt-6 flex items-center justify-center gap-8">
                <button className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-black/40 hover:text-[#C4141A]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6 mb-32">
          {[
            ...
            <div key={i} className="flex flex-col items-center p-8 bg-gray-50 border border-black/5 rounded-[2.5rem] group hover:bg-[#C4141A] transition-all duration-500">
              <item.icon size={20} className="text-[#C4141A] mb-4 group-hover:text-white transition-colors" />
                  ...
                  {Object.entries(car.features)
                    ...
                    .map(([key, _]) => (
                      <div key={key} className="flex items-center gap-4 py-2">
                        <div className="w-2 h-2 rounded-full bg-[#C4141A]" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20">
              {[
                ...
                <div key={i} className="flex gap-6 p-8 bg-gray-50 rounded-[2.5rem] border border-black/5">
                  <div className="w-12 h-12 bg-[#C4141A] rounded-2xl flex items-center justify-center text-white shrink-0 shadow-lg">
                    <ShieldCheck size={24} />
                  </div>
                  <div>
            ...
            >
              ...
              <form onSubmit={handleInquiry} className="space-y-6">
                <input 
                  ...
                  className="w-full bg-gray-50 border border-black/5 p-6 rounded-2xl text-black text-xs font-bold uppercase outline-none focus:border-[#C4141A] transition-all"
