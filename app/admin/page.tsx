    ...
    <div className="space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {stats.map((stat, i) => (
          ...
          >
            <div className={`p-4 rounded-2xl bg-white text-[#C4141A] shadow-[0_0_20px_rgba(255,255,255,0.2)] group-hover:scale-110 transition-transform`}>
              ...
              inquiries.slice(0, 5).map((inquiry) => (
                <div key={inquiry.id} className="flex items-center justify-between py-6 border-b border-white/5 last:border-0 group">
                  <div className="flex items-center gap-6">
                    <div className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center text-white border border-white/10 group-hover:bg-white group-hover:text-[#C4141A] transition-all">
          ...
          <div className="space-y-8">
            {cars.slice(0, 5).map((car) => (
              <div key={car.id} className="flex items-center justify-between py-6 border-b border-white/5 last:border-0 group">
                ...
                <span className="px-3 py-1 bg-white/10 text-white text-[8px] uppercase tracking-[0.3em] font-black border border-white/20 rounded-full group-hover:bg-white group-hover:text-[#C4141A] transition-all">Verified</span>
              </div>
            ))}
          </div>
        </section>
