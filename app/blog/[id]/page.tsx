  ...
  return (
    <main className="min-h-screen bg-white flex flex-col">
      ...
      <div className="pt-64 pb-20 px-6 max-w-5xl mx-auto w-full text-center">
        <button 
          onClick={() => router.back()}
          className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] text-[#C4141A] hover:text-black transition-all mb-12 mx-auto group"
        >
        ...
        >
          <span className="text-[10px] font-black uppercase tracking-[0.5em] text-[#C4141A] mb-6 block">
          ...
          <div className="flex flex-wrap items-center justify-center gap-8 text-[9px] font-black uppercase tracking-widest text-black/40">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-[#C4141A] text-white flex items-center justify-center text-[10px]">
                {blog.author?.charAt(0).toUpperCase() || "O"}
              </div>
              <span>By {blog.author || "OM Editor"}</span>
            </div>
            <div className="flex items-center gap-2"><Calendar size={14} className="text-[#C4141A]" /> {new Date(blog.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</div>
            <div className="flex items-center gap-2"><Clock size={14} className="text-[#C4141A]" /> {blog.readTime || "5 MIN READ"}</div>
          </div>
        </motion.div>
      </div>
      ...
      <section className="px-6 max-w-4xl mx-auto w-full mb-32">
        <div className="flex flex-col md:flex-row gap-20">
          ...
          <aside className="md:w-12 shrink-0 flex md:flex-col gap-6 md:pt-4 border-b md:border-b-0 pb-10 md:pb-0 mb-10 md:mb-0">
            ...
            <button className="w-12 h-12 rounded-full border border-black/5 flex items-center justify-center text-black/40 hover:bg-[#C4141A] hover:text-white hover:border-[#C4141A] transition-all">
              <Facebook size={18} />
            </button>
            <button className="w-12 h-12 rounded-full border border-black/5 flex items-center justify-center text-black/40 hover:bg-[#C4141A] hover:text-white hover:border-[#C4141A] transition-all">
              <Twitter size={18} />
