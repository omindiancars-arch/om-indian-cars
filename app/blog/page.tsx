      ...
      <section className="pt-64 pb-24 px-6 bg-gray-50 border-b border-black/5">
        <div className="max-w-7xl mx-auto text-center">
          <motion.span 
            ...
            className="text-[10px] font-black uppercase tracking-[0.5em] text-[#A10B1C] mb-6 block"
          >
            Insights & Lifestyle
          </motion.span>
          <motion.h1 
          ...
          >
            The OM <br className="md:hidden" /><span className="text-[#A10B1C]">Journal</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
      ...
      <Link href={`/blog/${blog.id}`}>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-6 text-[8px] font-black uppercase tracking-widest text-black/40">
            <span className="flex items-center gap-2"><Calendar size={12} className="text-[#A10B1C]" /> {new Date(blog.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
            <span className="flex items-center gap-2"><Clock size={12} className="text-[#A10B1C]" /> {blog.readTime || "5 MIN READ"}</span>
          </div>
          
          <h3 className="text-2xl font-heading font-black uppercase tracking-tighter text-black group-hover:text-[#A10B1C] transition-colors line-clamp-2">
            {blog.title}
          </h3>
          ...
          <div className="pt-4 flex items-center gap-2 text-[#A10B1C] text-[9px] font-black uppercase tracking-[0.3em] group-hover:gap-4 transition-all">
