...
export default function EliteSearch() {
  ...
  const Dropdown = ({ label, value, options, onSelect, type }: any) => (
    <div className="flex-1 w-full relative group">
      <label className="absolute -top-3 left-6 px-3 bg-white text-[9px] font-black uppercase tracking-[0.2em] text-[#C50403] z-10 border border-black/5 rounded-full">
        {label}
      </label>
      <div 
      ...
      >
        ...
        <ChevronDown size={18} className={`text-[#C50403] transition-transform duration-300 ${activeDropdown === type ? "rotate-180" : ""}`} />
      </div>
        ...
        {activeDropdown === type && (
          ...
          >
            <div 
              ...
              className="p-4 hover:bg-[#C50403]/5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors cursor-pointer"
            >
              Any {label}
            </div>
            {options.map((opt: string) => (
              <div 
                ...
                className={`p-4 hover:bg-[#C50403]/5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors cursor-pointer ${value === opt ? 'bg-[#C50403]/5 text-[#C50403]' : 'text-black/60'}`}
              >
                {opt}
              </div>
            ))}
    ...
    <div ref={containerRef} className="w-full max-w-5xl mx-auto px-6 relative z-50">
      <div className="bg-white/95 backdrop-blur-xl p-5 md:p-6 rounded-[2.5rem] md:rounded-full shadow-[0_30px_100px_rgba(0,0,0,0.2)] flex flex-col md:flex-row items-center gap-5 md:gap-6 border border-white/20">
        ...
        <button 
          ...
          className="w-full md:w-24 h-16 md:h-20 rounded-2xl md:rounded-full bg-[#C50403] text-white flex items-center justify-center shadow-xl hover:bg-black hover:scale-105 active:scale-95 transition-all shrink-0"
        >
