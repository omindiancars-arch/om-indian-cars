      ...
      <Link href={`/car/${car.id}`} className="block h-full group relative">
        ...
        >
          ...
          <div className="relative aspect-[4/3] overflow-hidden bg-[#1a1a1a]">
            <AnimatePresence mode="wait">
              ...
              >
                <Image 
                  ...
                  alt={car.name}
                  fill
                  className="object-cover scale-[1.12] origin-top-left"
                  sizes="(max-width: 768px) 100vw, 33vw"
                />
              </motion.div>
            </AnimatePresence>

            {/* Minimalist Branding to mask watermark */}
            <div className="absolute bottom-3 right-3 w-8 h-8 z-20 pointer-events-none opacity-50 group-hover:opacity-80 transition-all duration-500">
            ...
            {images.length > 1 && (
              <>
                <button onClick={prevImage} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/50 backdrop-blur-md shadow-xl flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-all z-20 hover:bg-[#C4141A]">
                  <ArrowRight size={16} className="rotate-180" />
                </button>
                <button onClick={nextImage} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/50 backdrop-blur-md shadow-xl flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-all z-20 hover:bg-[#C4141A]">
                  <ArrowRight size={16} />
                </button>
              </>
            )}
            ...
            <button 
              ...
              className={`absolute top-3 md:top-6 right-3 md:right-6 w-8 h-8 md:w-12 md:h-12 rounded-full border flex items-center justify-center transition-all shadow-xl z-20 ${
                isInterested ? "bg-[#C4141A] text-white border-[#C4141A]" : "bg-black/50 backdrop-blur-md border-white/10 text-white/40 hover:text-white"
              }`}
            >
          ...
          <div className="p-4 md:p-10 lg:p-12 flex-1 flex flex-col bg-white text-black">
            ...
            <div className="flex flex-wrap gap-2 mb-6 md:mb-8">
              ...
              {car.services?.finance && (
                <div className="bg-[#C4141A] text-white text-[7px] md:text-[9px] font-black uppercase tracking-widest px-3 md:px-4 py-1.5 md:py-2 rounded-full flex items-center gap-1.5 md:gap-2 shadow-sm">
            ...
            <div className="mb-4 md:mb-8">
              <h3 className="text-[12px] md:text-3xl font-heading font-black uppercase tracking-tighter text-black leading-tight group-hover:text-[#C4141A] transition-colors duration-500">
            ...
            <div className="flex items-center justify-between py-4 md:py-6 border-y border-black/5 mb-4 md:mb-8">
              {[
                ...
                <div key={i} className="flex flex-col items-center">
                  <spec.icon className="w-3 h-3 md:w-4 md:h-4 text-[#C4141A] mb-1 md:mb-2 opacity-60" />
            ...
            <div className="mt-auto flex items-end justify-between">
              <div>
                <p className="text-sm md:text-4xl font-sans font-bold text-[#C4141A] tracking-tighter whitespace-nowrap flex items-center gap-2">
                  ₹ {car.price.replace(/[₹]/g, "").trim()}
                </p>
              </div>
              <div className="w-8 h-8 md:w-14 md:h-14 rounded-xl md:rounded-2xl bg-black/5 border border-black/10 flex items-center justify-center text-black/20 group-hover:bg-[#C4141A] group-hover:text-white transition-all duration-500">
          ...
          <AnimatePresence>
            {isInterested && (
              <motion.div 
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="absolute inset-0 z-30 bg-[#C4141A]/95 backdrop-blur-md flex flex-col items-center justify-center text-white p-8 text-center"
              >
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="text-left space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full bg-gray-50 border border-black/5 p-5 rounded-2xl text-black text-xs font-bold uppercase outline-none focus:border-[#C4141A] transition-all"
                    />
                  </div>
                  <div className="text-left space-y-2">
                  ...
                  <button 
                    type="submit"
                    className="w-full bg-[#C4141A] text-white py-6 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-black transition-all shadow-xl active:scale-95"
                  >
                    Confirm Callback
                  </button>
                </form>
